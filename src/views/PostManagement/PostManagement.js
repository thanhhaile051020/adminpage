...
const PostManagement = ({ match }) => {
  ...
  const handleSearchString = (selectedKeys, confirm, dataIndex) => {
    confirm();
    setSearchText(selectedKeys);
    setSearchedColumn(dataIndex);
  };
  const handleResetString = (clearFilters) => {
    clearFilters();
    setSearchText("");
  };
  const getColumnSearchStringProps = (dataIndex) => ({
    filterDropdown: ({
      setSelectedKeys,
      selectedKeys,
      confirm,
      ...
      <div style={{ padding: 8 }}>
        <Input
          ...
          onPressEnter={() => handleSearchString(selectedKeys, confirm, dataIndex)}
          style={{ marginBottom: 8, display: "block" }}
        />
        <Space>
          <Button
            type="primary"
            onClick={() => handleSearchString(selectedKeys, confirm, dataIndex)}
            icon={<SearchOutlined />}
            size="small"
            style={{ width: 90 }}
          >
          ...
          <Button
            onClick={() => handleResetString(clearFilters)}
            size="small"
            style={{ width: 90 }}
          >
            Reset
  ...
  const columns = [
    {
      ...
      ...getColumnSearchStringProps("_id")
    },
    {
      title: "Username",
      key: "posterId",
